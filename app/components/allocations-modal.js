...
const ROLES = [
  { name: 'Principal', id: 'role:principal', group: 'Roles' },
...
let editableProps = EDITABLE_PROPERTIES.reduce(
  function(props, name) {
    props[name] = null;
    return props;
  },
...
export default T2Modal.extend(editableProps, FormDateRangeMixin, {
  ...
  peopleAndRoles: Ember.computed('project', function() {
    ...
    peopleAndRoles.pushObjects(
      this.get('people').map(person => ({
  ...
  percentAllocatedObserver: Ember.observer(
    'personOrRoleSelection',
    'project',
    function() {
  ...
  projects: Ember.computed.filter(
    'sortedProjects',
    item => item.get('name') !== 'Available'
  ),
  ...
  errors: Ember.computed('_editedModel.errors.[]', function() {
    ...
    this.get('_editedModel.errors').forEach(error =>
  ...
  getPersonOrRole(allocation) {
    return this.get('peopleAndRoles').find(personOrRole => {
      return allocation.get('person.content.id') === personOrRole.id ||
        allocation.get('role') === personOrRole.name;
    });
  },
  ...
  _applyChanges(allocation) {
    ...
    this.get('projectsController').set(
      'lastLikelihood',
      this.get('likelihood')
    );
    return this.get('projectsController').set(
      'lastEndDate',
      this.get('endDate')
    );
  }
